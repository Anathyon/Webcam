...
const video = getElement<HTMLVideoElement>('#webcam-video');
const canvas = getElement<HTMLCanvasElement>('#webcam-canvas');
const webcamOverlay = getElement<HTMLDivElement>('#webcam-overlay');
const filtroNomeDesktop = getElement<HTMLSpanElement>('#filtro-nome');
const filtroStatusOverlay = getElement<HTMLSpanElement>('#filtro-status-overlay');
const filterListUl = getElement<HTMLUListElement>('#filter-list');
const filterCountSpan = getElement<HTMLSpanElement>('#filter-count');
const btnCapturar = getElement<HTMLButtonElement>('#btn-capturar');
const btnGravar = getElement<HTMLButtonElement>('#btn-gravar');
const btnAlternar = getElement<HTMLButtonElement>('#btn-alternar');
const btnTema = getElement<HTMLButtonElement>('#btn-modo-claro');
const themeIcon = getElement<HTMLElement>('#theme-icon');
const headerLogo = getElement<HTMLImageElement>('#header-logo');
const galleryModal = getElement<HTMLDialogElement>('#gallery-modal');
const btnAbrirGaleriaDesktop = getElement<HTMLButtonElement>('#btn-abrir-galeria-desktop');
const btnAbrirGaleriaPreview = getElement<HTMLButtonElement>('#btn-abrir-galeria-preview');
const btnFecharModal = getElement<HTMLButtonElement>('#btn-fechar-modal');
const fotosGrid = getElement<HTMLDivElement>('#fotos-grid');
const videosGrid = getElement<HTMLDivElement>('#videos-grid');
const fotosCount = getElement<HTMLSpanElement>('#fotos-count');
const videosCount = getElement<HTMLSpanElement>('#videos-count');
const tabFotos = getElement<HTMLButtonElement>('#tab-fotos');
const tabVideos = getElement<HTMLButtonElement>('#tab-videos');
const mediaPreviewModal = getElement<HTMLDialogElement>('#media-preview-modal');
const btnFecharPreview = getElement<HTMLButtonElement>('#btn-fechar-preview');
const previewMedia = getElement<HTMLDivElement>('#preview-media');
const previewTitle = getElement<HTMLHeadingElement>('#preview-title');
const previewDate = getElement<HTMLParagraphElement>('#preview-date');
const btnPreviewDownload = getElement<HTMLButtonElement>('#btn-preview-download');
const btnPreviewShare = getElement<HTMLButtonElement>('#btn-preview-share');
const btnPreviewDelete = getElement<HTMLButtonElement>('#btn-preview-delete');
const galleryPreview = getElement<HTMLDivElement>('#gallery-preview');
const capturasCount = getElement<HTMLParagraphElement>('#capturas-count');
const lastPhotoTime = getElement<HTMLParagraphElement>('#last-photo-time');
const recordingIndicator = getElement<HTMLDivElement>('#recording-indicator');
const recordingControls = getElement<HTMLDivElement>('#recording-controls');
const recordingTimer = getElement<HTMLDivElement>('#recording-timer');
const btnPauseRecording = getElement<HTMLButtonElement>('#btn-pause-recording');
const btnStopRecording = getElement<HTMLButtonElement>('#btn-stop-recording');
...
const isValidDataUrl = (url: string): boolean => 
    /^data:image\/(png|jpeg|jpg|gif|webp);base64,/.test(url);

const isValidVideoUrl = (url: string): boolean => 
    /^data:video\/(mp4|webm|ogg);base64,/.test(url) || url.startsWith('blob:');

const showSecureNotification = (message: string, type: 'success' | 'error' = 'success'): void => {
...
const aplicarFiltro = (index: number, updateGlobalIndex = true): void => {
...
const updateFilterStatus = (nome: string): void => {
...
const getFotos = (): Foto[] => {
    try {
        ...
        return parsed.filter((foto: any): foto is Foto => 
            foto && 
...
const getVideos = (): Video[] => {
    try {
        ...
        return parsed.filter((video: any): video is Video => 
            video && 
...
const getAllMedia = (): MediaItem[] => {
    const fotos = getFotos().map(f => ({ ...f, type: 'foto' as const }));
    const videos = getVideos().map(v => ({ ...v, type: 'video' as const }));
    return [...fotos, ...videos].sort((a, b) => b.timestamp - a.timestamp);
};

const renderizarPreview = (): void => {
    const allMedia = getAllMedia();
    if (!galleryPreview || !lastPhotoTime || !capturasCount) return;
...
const salvarVideo = (dataUrl: string, duration: number, format: string = 'webm'): void => {
    try {
...
let activeTab: 'fotos' | 'videos' = 'fotos';
let currentPreviewItem: MediaItem | null = null;
...
const renderizarGaleria = (): void => {
    ...
    if (fotosCount) fotosCount.textContent = fotos.length.toString();
    if (videosCount) videosCount.textContent = videos.length.toString();
    
    // Converter para MediaItem e renderizar
    const fotosAsMedia = fotos.map(f => ({ ...f, type: 'foto' as const }));
    const videosAsMedia = videos.map(v => ({ ...v, type: 'video' as const }));
...
const renderizarGrid = (items: MediaItem[], grid: HTMLElement | null, type: 'foto' | 'video'): void => {
    if (!grid) return;
    ...
    items.forEach(item => {
        ...
        if (type === 'foto') {
            ...
            info.textContent = formatDuration((item as Video).duration || 0);
...
const switchTab = (tab: 'fotos' | 'videos'): void => {
    activeTab = tab;
...
const openMediaPreview = (item: MediaItem): void => {
    if (!mediaPreviewModal || !previewMedia || !previewTitle || !previewDate) return;
    ...
    if (item.type === 'foto') {
        ...
        previewTitle.textContent = `Vídeo (${formatDuration((item as Video).duration || 0)})`;
    }
...
const baixarVideo = (dataUrl: string): void => {
    try {
        const link = document.createElement('a');
        link.href = dataUrl;
...
const deletarFoto = (id: string): void => {
...
const deletarVideo = (id: string): void => {
    if (!id || typeof id !== 'string') {
...
const compartilharFoto = async (dataUrl: string): Promise<void> => {
    if (!isValidDataUrl(dataUrl)) {
        showSecureNotification('URL de imagem inválida', 'error');
        return;
    }
...
const compartilharVideo = async (dataUrl: string): Promise<void> => {
    try {
        if (!navigator.share) {
            showSecureNotification('Compartilhamento não suportado neste navegador.', 'error');
            return;
...
const getSupportedMimeType = (withAudio = true): { mimeType: string; supportsAudio: boolean } => {
...
import { VideoConfig, defaultVideoConfig, getQualitySettings, getOptimalVideoConfig } from './video-config.js';

// Variáveis para MP4 recording
let mp4Recorder: SimpleMP4VideoRecorder | null = null;
let currentVideoConfig: VideoConfig = getOptimalVideoConfig();

// Elementos do modal de configuração
const videoConfigModal = getElement<HTMLDialogElement>('#video-config-modal');
const btnVideoConfig = getElement<HTMLButtonElement>('#btn-video-config');
const btnFecharConfigModal = getElement<HTMLButtonElement>('#btn-fechar-config-modal');
const btnSaveConfig = getElement<HTMLButtonElement>('#btn-save-config');
const btnResetConfig = getElement<HTMLButtonElement>('#btn-reset-config');
...
const iniciarGravacaoMP4 = async (): Promise<void> => {
    if (!streamAtual || !video) return;
...
const pararGravacaoMP4 = async (): Promise<void> => {
    if (!mp4Recorder || !isRecording) return;
...
const showRecordingControls = (): void => {
...
const hideRecordingControls = (): void => {
    if (recordingIndicator) {
...
const updateTimerDisplay = (seconds: number): void => {
    if (recordingTimer) {
        const mins = Math.floor(seconds / 60);
        const secs = Math.floor(seconds % 60);
...
const renderizarListaFiltros = (): void => {
    if (!filterListUl || !filterCountSpan) return;
...
const updateConfigModal = (): void => {
    // Atualizar formato
    const formatMP4 = getElement<HTMLInputElement>('#format-mp4');
    const formatWebM = getElement<HTMLInputElement>('#format-webm');
    ...
    // Atualizar qualidade
    const qualityInputs = {
        low: getElement<HTMLInputElement>('#quality-low'),
        medium: getElement<HTMLInputElement>('#quality-medium'),
        high: getElement<HTMLInputElement>('#quality-high')
    };
...
const readConfigFromModal = (): void => {
    // Ler formato
    const formatMP4 = getElement<HTMLInputElement>('#format-mp4');
    if (formatMP4) {
    ...
    if (qualityInputs.length > 0) {
        const selectedQuality = (qualityInputs[0] as HTMLInputElement).value as VideoConfig['quality'];
        currentVideoConfig.quality = selectedQuality;
    }
};

const resetVideoConfig = (): void => {
    currentVideoConfig = getOptimalVideoConfig();
    updateConfigModal();
...
const handleOrientationChange = (): void => {
    setTimeout(() => {
        ...
        const container = getElement<HTMLDivElement>('.webcam-container');
        if (container) {
...
const setupSecureEventListeners = (): void => {
    ...
    btnPauseRecording?.addEventListener('click', (e): void => {
        e.preventDefault();
    ...
    btnStopRecording?.addEventListener('click', (e): void => {
        e.preventDefault();
