class MP4VideoRecorder {
    ...
    async processVideo() {
        ...
        if (this.audioChunks.length > 0) {
            ...
            await this.ffmpeg.run('-i', 'video.webm', '-i', 'audio.webm', '-c:v', 'libx264', '-c:a', 'aac', '-preset', 'fast', '-crf', '23', 'output.mp4');
        }
        else {
            // Apenas vídeo para MP4
            await this.ffmpeg.run('-i', 'video.webm', '-c:v', 'libx264', '-preset', 'fast', '-crf', '23', 'output.mp4');
        }
